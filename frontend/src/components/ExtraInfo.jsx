...
const ExtraInfo = ({ formData, setFormData }) => {
  ...
  // ✅ Debugging: Log form data updates
  useEffect(() => {
    console.log("✅ Updated formData in ExtraInfo:", formData);
  }, [formData]);

  // Handle extra staff input
  const handleStaffChange = (e) => {
    const value = Math.max(0, parseInt(e.target.value, 10) || 0);
    setExtraStaff(value);
    setFormData((prev) => ({ ...prev, extraStaff: value }));
  };
        ...
        <label htmlFor="toggle" className="flex items-center cursor-pointer">
          <div className="relative">
            <input
              ...
              onChange={(e) =>
                setFormData((prev) => ({ ...prev, hasDependents: e.target.checked }))
              }
            />
            <div
      ...
      <form className="flex flex-col items-center justify-center space-y-[1rem] mt-[2rem]">
        <div className="w-[50rem]">
          ...
          <select
            ...
            value={formData?.hasCleaningEquipment ? "Yes" : "No"}
            onChange={(e) =>
              setFormData((prev) => ({ ...prev, hasCleaningEquipment: e.target.value === "Yes" }))
            }
